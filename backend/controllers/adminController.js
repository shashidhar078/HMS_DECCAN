...
// Approve Lab Technician
const approveLabTechnician = async (req, res) => {
    try {
        const { id } = req.params;
        const labTechnician = await User.findByIdAndUpdate(
            id,
            { isApproved: true, status: "Approved" },
            { new: true }
        );
        ...
        const welcomeEmail = `
            <h1>Welcome to Hospital Management System</h1>
            <p>Dear ${labTechnician.username},</p>
            <p>Your registration has been approved by the admin. You can now log in to your account.</p>
            <p>Your login credentials:</p>
            <ul>
                <li>Email: ${labTechnician.email}</li>
                <li>Role: Lab Technician</li>
            </ul>
            <p>Please log in to complete your profile setup.</p>
            <p>Best regards,<br>Hospital Management System</p>
        `;
        ...
        res.status(200).json({
            message: "Lab technician approved successfully",
            labTechnician: {
                id: labTechnician._id,
        ...
        console.error("Approve lab technician error:", error);
        res.status(500).json({ message: error.message });
    }
};

// Reject Lab Technician
const rejectLabTechnician = async (req, res) => {
    try {
        console.log('Starting lab technician rejection process...');
        const { id } = req.params;
        const labTechnician = await User.findById(id);

        if (!labTechnician) {
            console.log('Lab technician not found with ID:', id);
            return res.status(404).json({ message: "Lab technician not found" });
        }

        console.log('Lab technician found for rejection:', {
            id: labTechnician._id,
            email: labTechnician.email,
            username: labTechnician.username
        });
        ...
        const rejectionEmail = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <h1 style="color: #2c3e50; text-align: center;">Registration Status Update</h1>
                <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin-top: 20px;">
                    <p style="font-size: 16px;">Dear ${labTechnician.username},</p>
                    <p style="font-size: 16px;">We regret to inform you that your registration request has been reviewed and unfortunately, we are unable to approve it at this time.</p>
                    <p style="font-size: 16px;">If you believe this is a mistake or would like to discuss this further, please contact our hospital administration at:</p>
                    <ul style="font-size: 16px;">
                        <li>Email: admin@hospital.com</li>
        ...
        console.log('Attempting to send rejection email to:', labTechnician.email);
        
        try {
            const emailResult = await transporter.sendMail({
                from: `"Hospital Management System" <${process.env.EMAIL_USER}>`,
                to: labTechnician.email,
                subject: "Registration Status Update - Hospital Management System",
                html: rejectionEmail
            });
        ...
        // Delete the lab technician's account
        await User.findByIdAndDelete(id);

        res.status(200).json({ 
            message: "Lab technician rejected and account deleted",
            emailSent: true
        });
    } catch (error) {
        console.error("Reject lab technician error:", error);
        res.status(500).json({ 
            message: error.message,
            error: error
        });
...
module.exports = { adminLogin, getPendingStaff, approveStaff, rejectStaff, getAllUsers, registerStaff, approveDoctor, rejectDoctor, approveReceptionist, rejectReceptionist, approveLabTechnician, rejectLabTechnician };
